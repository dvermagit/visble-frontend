...
export default function InlineCalendly() {
  ...
  /* -------------------------------------------------- */
  /* Preload Calendly ASAP (fixes slowness) */
  /* -------------------------------------------------- */
  useEffect(() => {
  ...
  const openCalendly = () => {
    ...
    // Mobile → popup (much more reliable)
    if (isMobile && window.Calendly) {
  ...
  /* -------------------------------------------------- */
  /* Desktop Inline Init (NO polling) */
  /* -------------------------------------------------- */
  useEffect(() => {
    if (!open || isMobile || !window.Calendly) return;

    const container = document.getElementById('calendly-inline');

    if (container) {
      container.innerHTML = ''; // IMPORTANT: reset before init
  ...
  return (
    <>
      ...
      {open && isMobile && (
        <div className="mt-6 text-center text-sm text-gray-600">
          Calendly opened in a popup.
          <br />
          If blocked, allow popups and try again.
        </div>
      )}
    </>
  );
